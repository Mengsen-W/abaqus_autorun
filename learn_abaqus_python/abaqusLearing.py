...
parameter = (
    ("Model Name:", "Model-NewShearWall"),
    ("Cover(mm):", "24"),
    ("Length(wall)(mm):", "1200"),
    ("Thickness(wall)(mm):", "240"),
    ("Height(wall)(mm):", "1050"),
    ("Diameter(tube)(mm):", "114"),
    ("Thickness(tube)(mm):", "4"),
...
modelname, cover, lwall, twall, hwall, dtube, ttube, dlbar, dtbar, lfd, tfd, hfd, lbl, tbl, hbl, fylbar, fytbar, fytube, dis, axld, offset1, offset2, space, meshsize, jobname = getInputs(
    fields=parameter, label="Please Input The Parameter", dialogTitle="Parameter Input"
)

viewportshow = session.Viewport(
    name="Viewport: 1", origin=(0.0, 0.0), width=500, height=200
)
...
partfoundation = myModel.Part(
    name="foundation", dimensionality=THREE_D, type=DEFORMABLE_BODY
)
...
partloadbeam = myModel.Part(
    name="loadbeam", dimensionality=THREE_D, type=DEFORMABLE_BODY
)
...
s.setPrimaryObject(option=STANDALONE)
s.rectangle(
    point1=(-(tbl / 2 - cover), -(hbl / 2 - cover)),
...
s.setPrimaryObject(option=STANDALONE)
s.rectangle(
    point1=(-hooplength / 2, -hoopwidth / 2), point2=(hooplength / 2, hoopwidth / 2)
)
...
s.setPrimaryObject(option=STANDALONE)
s.rectangle(
    point1=(-hooplength2 / 2, -hoopwidth / 2), point2=(hooplength2 / 2, hoopwidth / 2)
)
...
# section
mdb.models["Model-NewShearWall"].HomogeneousSolidSection(
...
mdb.models["Model-NewShearWall"].HomogeneousSolidSection(
    name="tube", material="tube", thickness=None
)
mdb.models["Model-NewShearWall"].TrussSection(
    name="rebar-l", material="rebar-l", area=3.1415926 / 4 * dlbar * dlbar
)
mdb.models["Model-NewShearWall"].TrussSection(
    name="rebar-t", material="rebar-t", area=3.1415926 / 4 * dtbar * dtbar
)
mdb.models["Model-NewShearWall"].TrussSection(
    name="rebar-rigid", material="rebar-rigid", area=100
)  # 此处属性暂按S=100计，以实际为准
...
for i in range(1, midhoopnum + 1):
    ...
    a.rotate(
        instanceList=("lhoop3-" + str(i),),
        axisPoint=(0.0, 0.0, 0.0),
        axisDirection=(0.0, 1, 0.0),
        angle=-90.0,
    )
    a.rotate(
        instanceList=("fhoop5-" + str(i),),
        axisPoint=(0.0, 0.0, 0.0),
        axisDirection=(0.0, 1, 0.0),
        angle=90.0,
    )
    a.rotate(
        instanceList=("fhoop6-" + str(i),),
        axisPoint=(0.0, 0.0, 0.0),
        axisDirection=(0.0, 1, 0.0),
        angle=90.0,
    )
...
for i in range(1, lhoopnum + 1):
    a = mdb.models["Model-NewShearWall"].rootAssembly
    a.rotate(
        instanceList=("lhoop1-" + str(i),),
        axisPoint=(0.0, 0.0, 0.0),
        axisDirection=(0.0, 1, 0.0),
        angle=-90.0,
    )
    a.rotate(
        instanceList=("lhoop2-" + str(i),),
        axisPoint=(0.0, 0.0, 0.0),
        axisDirection=(0.0, 1, 0.0),
        angle=-90.0,
    )
...
for i in range(1, fhoopnum + 1):
    a = mdb.models["Model-NewShearWall"].rootAssembly
    a.rotate(
        instanceList=("fhoop1-" + str(i),),
        axisPoint=(0.0, 0.0, 0.0),
        axisDirection=(0.0, 1, 0.0),
        angle=90.0,
    )
    a.rotate(
        instanceList=("fhoop2-" + str(i),),
        axisPoint=(0.0, 0.0, 0.0),
        axisDirection=(0.0, 1, 0.0),
        angle=90.0,
    )
    a.rotate(
        instanceList=("fhoop3-" + str(i),),
        axisPoint=(0.0, 0.0, 0.0),
        axisDirection=(0.0, 1, 0.0),
        angle=90.0,
    )
    a.rotate(
        instanceList=("fhoop4-" + str(i),),
        axisPoint=(0.0, 0.0, 0.0),
        axisDirection=(0.0, 1, 0.0),
        angle=90.0,
    )
...
a.translate(
    instanceList=("lsteel-1",),
    vector=(-(offset1 * 1.5 - offset2 / 2), twall / 2 - cover, cover),
)
a.translate(
    instanceList=("lsteel-2",),
    vector=(offset1 * 1.5 - offset2 / 2, twall / 2 - cover, cover),
)
a.translate(
    instanceList=("lsteel-3",),
    vector=(-(offset1 * 1.5 - offset2 / 2), -(twall / 2 - cover), cover),
)
a.translate(
    instanceList=("lsteel-4",),
    vector=(offset1 * 1.5 - offset2 / 2, -(twall / 2 - cover), cover),
)

a.translate(
    instanceList=("lsteel-5",),
    vector=(-(offset1 + offset2) / 2, twall / 2 - cover, cover),
)
a.translate(
    instanceList=("lsteel-6",),
    vector=((offset1 + offset2) / 2, twall / 2 - cover, cover),
)
a.translate(
    instanceList=("lsteel-7",),
    vector=(-(offset1 + offset2) / 2, -(twall / 2 - cover), cover),
)
a.translate(
    instanceList=("lsteel-8",),
    vector=((offset1 + offset2) / 2, -(twall / 2 - cover), cover),
)

a.translate(
    instanceList=("lsteel-9",),
    vector=(-(offset2 * 1.5 - offset1 / 2), twall / 2 - cover, cover),
)
a.translate(
    instanceList=("lsteel-10",),
    vector=(offset2 * 1.5 - offset1 / 2, twall / 2 - cover, cover),
)
a.translate(
    instanceList=("lsteel-11",),
    vector=(-(offset2 * 1.5 - offset1 / 2), -(twall / 2 - cover), cover),
)
a.translate(
    instanceList=("lsteel-12",),
    vector=(offset2 * 1.5 - offset1 / 2, -(twall / 2 - cover), cover),
)
...
for i in range(1, hoopnum + 1):
    a.translate(
        instanceList=("hoop11-" + str(i),), vector=(offset1, 0, cover + (i - 1) * space)
    )
    a.translate(
        instanceList=("hoop12-" + str(i),),
        vector=(-offset1, 0, cover + (i - 1) * space + dtbar),
    )
    a.translate(
        instanceList=("hoop13-" + str(i),),
        vector=(offset2, 0, cover + (i - 1) * space + dtbar),
    )
    a.translate(
        instanceList=("hoop14-" + str(i),),
        vector=(-offset2, 0, cover + (i - 1) * space),
    )
    a.translate(
        instanceList=("hoop21-" + str(i),),
        vector=((hooplength2) / 2, 0, cover + (i - 1) * space),
    )
    a.translate(
        instanceList=("hoop22-" + str(i),),
...
a.translate(
    instanceList=("f-lsteel-1",), vector=(-lfd / 2 + cover, tfd / 2 - cover, -cover)
)
a.translate(
    instanceList=("f-lsteel-2",), vector=(-lfd / 2 + cover, -(tfd / 2 - cover), -cover)
)
a.translate(
    instanceList=("f-lsteel-3",),
    vector=(-lfd / 2 + cover, -(tfd / 2 - cover), -hfd + cover),
)
a.translate(
    instanceList=("f-lsteel-4",),
    vector=(-lfd / 2 + cover, tfd / 2 - cover, -hfd + cover),
)
a.translate(
    instanceList=("f-lsteel-5",),
    vector=(-lfd / 2 + cover, -(dtube / 2 + cover), -cover),
)
a.translate(
    instanceList=("f-lsteel-6",), vector=(-lfd / 2 + cover, dtube / 2 + cover, -cover)
)
a.translate(
    instanceList=("f-lsteel-7",),
    vector=(-lfd / 2 + cover, dtube / 2 + cover, -hfd + cover),
)
a.translate(
    instanceList=("f-lsteel-8",),
    vector=(-lfd / 2 + cover, -(dtube / 2 + cover), -hfd + cover),
)

a.translate(
    instanceList=("l-lsteel-1",),
    vector=(-lbl / 2 + cover, -(tbl / 2 - cover), hwall + hbl - cover),
)
a.translate(
    instanceList=("l-lsteel-2",),
    vector=(-lbl / 2 + cover, tbl / 2 - cover, hwall + hbl - cover),
)
a.translate(
    instanceList=("l-lsteel-3",),
    vector=(-lbl / 2 + cover, -(tbl / 2 - cover), hwall + cover),
)
a.translate(
    instanceList=("l-lsteel-4",),
    vector=(-lbl / 2 + cover, tbl / 2 - cover, hwall + cover),
)
...
mdb.models["Model-NewShearWall"].EmbeddedRegion(
    name="steel-embed",
...
s = mdb.models["Model-NewShearWall"].ConstrainedSketch(
    name="__profile__", sheetSize=4000, gridSpacing=50, transform=t
)
...
# step
mdb.models["Model-NewShearWall"].StaticStep(
    name="Step-1",
    previous="Initial",
    timePeriod=1.0,
    maxNumInc=500,
...
mdb.models["Model-NewShearWall"].Coupling(
    name="axial-loading",
...
mdb.models["Model-NewShearWall"].ConcentratedForce(
    name="axial-loading",
...
# load
mdb.models["Model-NewShearWall"].TabularAmplitude(
    name="Amp-1",
    timeSpan=STEP,
...
mdb.models["Model-NewShearWall"].DisplacementBC(
    name="lateral-loading",
...
mdb.models["Model-NewShearWall"].EncastreBC(
    name="bottom-encastre", createStepName="Initial", region=region, localCsys=None
)
